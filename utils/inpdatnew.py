...
def parse_inp_to_dat(inp_filepath, dat_filepath):
        ...
        for local_elem_id_iter, elem_def_iter in part_data_ref["elements"].items():
            ...
            if elem_material_name_str and elem_material_name_str in inp_data["materials_global"]:
                ...
                if elem_section_info:
                    if elem_section_info["type"] == "Solid" and elem_abaqus_type_str in ["T3D2",
                                                                                         "C3D8R"]:  # Solid Section也用于C3D8R
                        if elem_section_info.get("props"):
    ...
    # 【新增】更新全局单元数据中的节点ID为合并后的新ID
    for elem_data_out in global_elements_data:
    ...
    for mat_key_out, group_elements_list in dat_element_groups_dict.items():
        ...
        for elem_data_out in group_elements_list:
            # elem_data_out["nodes"] has been updated
    ...
    return {
        ...
        "num_loads": len(loads_to_write)
    }


# --- 主执行块 ---
# 使用你提供的文件路径
inp_file_path = 'Bridge-4.inp'
dat_file_path = 'Bridge-4.dat'
...
if conversion_summary_data['nlcase'] > 0:
    print(f"总集中荷载条目数 (NL): {conversion_summary_data['num_loads']}")
...
if conversion_summary_data['nlcase'] > 0:
    print("INP 文件中的 *Dload, GRAV 重力荷载已转换为 DAT 文件中的节点集中荷载。")
else:
    print("INP 文件中未发现或未处理任何荷载。")
