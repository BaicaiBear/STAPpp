...
def parse_inp_to_dat(inp_filepath, dat_filepath):
    ...
    global_nodes_coords_bcs = {}  # global_node_id -> {"coords": [], "bcode": [0,0,0,0,0,0]}
    global_elements_data = []  # 包含单元字典的列表 (global_id, type, global_nodes, material_key)

    # 定义DOF映射 - 根据单元类型决定节点的DOF
    dof_map = {
        1: [0, 0, 0, 1, 1, 1],  # T3D2: xyz自由
        2: [0, 0, 0, 1, 1, 1],  # C3D8R: xyz自由
        3: [1, 1, 0, 0, 0, 1],  # S4R: z自由, theta_x, theta_y自由
        4: [0, 0, 0, 0, 0, 0],  # B31: 全部自由
    }
    
    abaqus_to_dat_type_map_dict = {'T3D2': 1, 'S4R': 3, 'C3D8R': 2, 'B31': 4}
    ...
    for inst in inp_data["assembly"]["instances"]:
        ...
        for local_elem_id_iter, elem_def_iter in part_data_ref["elements"].items():
            ...
            # 根据单元类型更新节点的bcode
            dat_elem_type_id = abaqus_to_dat_type_map_dict.get(elem_abaqus_type_str)
            if dat_elem_type_id and dat_elem_type_id in dof_map:
                element_dof_constraint = dof_map[dat_elem_type_id]
                for gn_id in global_node_ids_for_elem_iter:
    ...
    # --- 合并重复坐标的节点 ---
    print("合并重复坐标的节点...")
    coord_to_node_id = {}  # coordinate tuple -> original global node id
    old_to_new_node_map = {}  # old_global_node_id -> new_global_node_id
    new_global_nodes_coords_bcs = {}  # new_node_id -> {"coords": [], "bcode": []}
    
    tolerance = 1e-6  # 坐标容差
    new_node_id = 1
    
    for old_node_id, node_data in global_nodes_coords_bcs.items():
        coords = node_data["coords"]
        bcode = node_data["bcode"]
        
        # 创建坐标的哈希键（使用四舍五入来处理浮点精度问题）
        coord_key = tuple(round(c / tolerance) * tolerance for c in coords)
        ...
        if coord_key in coord_to_node_id:
            ...
            old_to_new_node_map[old_node_id] = new_node_id
            new_global_nodes_coords_bcs[new_node_id] = {
                "coords": coords,
                "bcode": bcode.copy()
            }
            new_node_id += 1
    ...
    # 更新所有单元的节点编号
    for elem_data in global_elements_data:
        elem_data["nodes"] = [old_to_new_node_map[old_id] for old_id in elem_data["nodes"]]
    
    # 更新全局节点映射
    for (inst_name, local_id), old_global_id in global_nodes_map.items():
        global_nodes_map[(inst_name, local_id)] = old_to_new_node_map[old_global_id]
    
    # 更新全局节点坐标和边界条件数据
    global_nodes_coords_bcs = new_global_nodes_coords_bcs
    
    print(f"  合并后节点数: {len(global_nodes_coords_bcs)} (原节点数: {len(old_to_new_node_map)})")

    # 【新增】重力荷载转换
    nodal_loads = {}  # {global_node_id: [fx, fy, fz]}
    if "gravity" in inp_data["loads"]:
        print("计算重力荷载并转换为节点力...")
        grav_mag = inp_data["loads"]["gravity"]["magnitude"]
        grav_vec = inp_data["loads"]["gravity"]["components"]

        for elem in global_elements_data:
            mat_key = elem.get("material_key")
            if not mat_key: continue

            mat_name = mat_key[1]
            material_data = inp_data["materials_global"].get(mat_name)
            if not material_data or not material_data.get("density"):
                continue

            density = material_data["density"]
            elem_nodes_ids = elem["nodes"]
            elem_nodes_coords = [new_global_nodes_coords_bcs[node_id]["coords"] for node_id in elem_nodes_ids]

            volume = 0.0
            abaqus_type = elem["type"]
            section_props = mat_key[3]

            try:
                if abaqus_type in ['T3D2', 'B31']:  # 1D单元
                    p1, p2 = np.array(elem_nodes_coords[0]), np.array(elem_nodes_coords[1])
                    length = np.linalg.norm(p1 - p2)
                    area = 0.0
                    if abaqus_type == 'T3D2':
                        area = section_props[0] if section_props else 0.0
                    elif abaqus_type == 'B31':  # 箱型梁截面面积
                        if len(section_props) >= 6:
                            b, d, tf_top, tf_bot, tw_left, tw_right = section_props[:6]
                            area = b * d - (b - tw_left - tw_right) * (d - tf_top - tf_bot)
                    volume = area * length

                elif abaqus_type == 'S4R':  # 2D壳单元
                    p1, p2, p3, p4 = [np.array(c) for c in elem_nodes_coords]
                    area1 = 0.5 * np.linalg.norm(np.cross(p2 - p1, p3 - p1))
                    area2 = 0.5 * np.linalg.norm(np.cross(p3 - p1, p4 - p1))
                    thickness = section_props[0] if section_props else 0.0
                    volume = (area1 + area2) * thickness

                elif abaqus_type == 'C3D8R':  # 3D实体单元
                    p = [np.array(c) for c in elem_nodes_coords]

                    def tet_vol(p1, p2, p3, p4):
                        return abs(np.dot(p1 - p4, np.cross(p2 - p4, p3 - p4))) / 6.0

                    # 将六面体分解为5个四面体来计算体积（这是一个常用且较稳健的方法）
                    # 注意：这个分解依赖于Abaqus的C3D8R节点顺序 (底面1-4逆时针，顶面5-8逆时针)
                    volume = (tet_vol(p[0], p[1], p[3], p[4]) +
                              tet_vol(p[1], p[2], p[3], p[6]) +
                              tet_vol(p[3], p[4], p[6], p[7]) +
                              tet_vol(p[1], p[4], p[5], p[6]) +
                              tet_vol(p[1], p[3], p[4], p[6]))
            except Exception as e:
                print(f"  警告: 计算单元体积时出错: {e}, 单元 {elem['id']}")
                continue

            if volume > 0:
                mass = volume * density
                total_force = [mass * grav_mag * c for c in grav_vec]
                num_nodes = len(elem_nodes_ids)
                if num_nodes > 0:
                    force_per_node = [f / num_nodes for f in total_force]
                    for node_id in elem_nodes_ids:
                        if node_id not in nodal_loads:
                            nodal_loads[node_id] = [0.0, 0.0, 0.0]
                        nodal_loads[node_id][0] += force_per_node[0]
                        nodal_loads[node_id][1] += force_per_node[1]
                        nodal_loads[node_id][2] += force_per_node[2]

    # 【新增】准备要写入文件的荷载数据
    loads_to_write = []
    for node_id, forces in sorted(nodal_loads.items()):
        # 忽略非常小的力以保持文件整洁
        if abs(forces[0]) > 1e-9: loads_to_write.append(f"{node_id} 1 {forces[0]:.7e}")
        if abs(forces[1]) > 1e-9: loads_to_write.append(f"{node_id} 2 {forces[1]:.7e}")
        if abs(forces[2]) > 1e-9: loads_to_write.append(f"{node_id} 3 {forces[2]:.7e}")
    ...
    for bc_item in inp_data["boundaries"]:
        ...
        # 应用边界条件 - 边界条件的bcode为[1,1,1,1,1,1]
    ...
    # 【修改】根据是否有荷载决定 nlcase_val
    nlcase_val = 1 if loads_to_write else 0
    numeg_val = len(dat_element_groups_dict)
    ...
    with open(dat_filepath, 'w', encoding='utf-8') as f_out:
        ...
        for gn_id_out in sorted_global_node_ids_list:
            ...
            f_out.write(
                f"{gn_id_out}  {bcodes_out[0]} {bcodes_out[1]} {bcodes_out[2]} {bcodes_out[3]} {bcodes_out[4]} {bcodes_out[5]} {coords_out[0]:.7e} {coords_out[1]:.7e} {coords_out[2]:.7e}\n")

        # 【新增】写入荷载信息块
        if nlcase_val > 0:
            f_out.write(f"1 {len(loads_to_write)}\n")
            for load_line in loads_to_write:
        ...
        for mat_key_out, group_elements_list in dat_element_groups_dict.items():
            ...
            mat_name = mat_key_out[1]
            density_val = 0.0
            if mat_name in inp_data["materials_global"]:
                density_val = inp_data["materials_global"][mat_name].get("density", 0.0)
            mat_line_parts_list = ["1", f"{e_nu_tuple_out[0]:.7e}", f"{e_nu_tuple_out[1]:.3f}", f"{density_val:.7e}"]
            ...
            if dat_elem_type_id_out == 1:
                ...
                mat_line_parts_list.extend([f"{thickness_val:.7e}"])
            elif dat_elem_type_id_out == 4:  # B31梁单元
            ...
            elif dat_elem_type_id_out == 2:  # C3D8R实体单元
                pass  # 实体单元不需要额外的截面属性
